...
class ClassRename(obfuscator_category.IRenameObfuscator):
    ...
    def encrypt_identifier(self, identifier: str) -> str:
        if identifier in self.dict:
            ...
            while self.dictionary:
                identifier_obfuscated = random.choice(list(self.dictionary))
                self.dictionary.remove(identifier_obfuscated)
                self.dict[identifier] = identifier_obfuscated
                return identifier_obfuscated
            # If all identifiers contain '$', fallback to the original method
            field_md5 = util.get_string_md5(identifier)
            return "m{0}".format(field_md5.lower()[:8])
        else:
            # If the dictionary is empty, fall back to the original method
            field_md5 = util.get_string_md5(identifier)
            return "m{0}".format(field_md5.lower()[:8])
    ...
    def obfuscate(self, obfuscation_info: Obfuscation):
        ...
        try:
            ...
            self.load_dictionary('./dict_confusion/dict_confusion_classes.txt')
