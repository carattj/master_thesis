...
class MethodRename(obfuscator_category.IRenameObfuscator):
    ...
    def rename_method(self, method_name: str) -> str:
        if method_name in self.dict:
            return self.dict[method_name]
        elif self.dictionary:
            field_obfuscated = random.choice(list(self.dictionary))
            self.dict[method_name] = field_obfuscated
            self.dictionary.remove(field_obfuscated)
            return field_obfuscated
        else:
            # If the dictionary is empty, fall back to the original renaming method
            field_md5 = util.get_string_md5(method_name)
    ...
    def obfuscate(self, obfuscation_info: Obfuscation):
        ...
        try:
            ...
            self.load_dictionary('./dict_confusion/dict_confusion_methods.txt')
