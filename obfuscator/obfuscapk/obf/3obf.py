...
def run_command(apk_files, mode, input_dir, output_dir, stats_dir):
    ...
    for i, apk_file in enumerate(apk_files):
        print(f'## Mode - {mode[0]} + {mode[1]} + {mode[2]}')
        ...
        command = [
            ...
            "-w",
            os.path.join(output_dir, f'{mode[0]}_{mode[1]}_{mode[2]}'),
        ]
        ...
        try:
            ...
            write_to_file(os.path.join(stats_dir, f'{mode[0]}_{mode[1]}_{mode[2]}.csv'), f'{apk_file},{True},{duration:.2f}')
            print(f"### Successful (Time: {duration:.2f} seconds)")
        except subprocess.CalledProcessError as e:
            end_time = time.time()
            duration = end_time - start_time
            write_to_file(os.path.join(stats_dir, f'{mode[0]}_{mode[1]}_{mode[2]}.csv'), f'{apk_file},{False},{duration:.2f}')
            print(f"### Fail")
            continue
...
def process_apks(input_dir, output_dir, stats_dir, modes):
    ...
    for i, mode in enumerate(modes):
        print(f'\n# Mode {i} - {mode[0]} + {mode[1]} + {mode[2]}')
        write_to_file(os.path.join(stats_dir, f'{mode[0]}_{mode[1]}_{mode[2]}.csv'), 'hash,obfuscated,time')
...
def main():
    ...
    modes = [['FieldRename','MethodRename','ClassRename']]
